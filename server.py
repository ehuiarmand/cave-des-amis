                ...
                for _key in _GLOBAL_PATCH_KEYS:
                    ...
                    if _key == "casiers":
                        ...
                        if put_delta.get("casiers"):
                            ...
                            # Sauvegarde complète (non-delta) : ne pas persister les
                            # tombstones {id, siteId, _deleted:true} envoyés par le client
                            # (sinon un casier fantôme réapparaît côté superadmin global).
                        ...
                        # Sauvegarde complète (non-delta) d'une collection : retirer les
                        # tombstones {id, ..., _deleted:true} pour ne pas persister de
                        # lignes fantômes côté superadmin global (charges, consignes,
                        # creditRecoveries, purchaseOrders, etc. supprimées via patch
                        # sans _putDelta réapparaîtraient sinon).
                        _val = payload[_key]
                        if isinstance(_val, list):
                            _val = [r for r in _val if not (isinstance(r, dict) and r.get("_deleted"))]
                        current[_key] = _val
                if "pdjWorkDateBySite" in payload:
...
def _server_auto_close_site(site_id: str, d_str: str) -> None:
    ...
    with store._lock:
        ...
        for item in site_stock:
            ...
            expected_remaining = frigo + reserve
